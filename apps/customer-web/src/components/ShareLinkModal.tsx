...
interface ShareTarget {
  ...
  className: string;
  // The web-intent URL to open when the native share sheet isn't used.
  href?: string;
  // mailto: links must be invoked in-place, never in a blank tab.
  scheme?: 'mailto';
  // Messenger has no reliable desktop deep link without a Facebook App ID, so
  // it falls back to copying the invite text and opening the app's site.
...
export const ShareLinkModal: React.FC<{ url: string; title: string; onClose: () => void }> = ({
  ...
  const encodedIntro = encodeURIComponent(`You're invited to ${title}! RSVP here:`);

  const shareTargets: ShareTarget[] = [
    { label: 'WhatsApp', Icon: WhatsAppIcon, className: 'bg-[#25D366] hover:brightness-110 text-white', href: `https://wa.me/?text=${encodedMessage}` },
    // The Facebook Send dialog needs an app_id + public URL, which we don't
    // have, so Messenger uses the copy-then-open fallback instead of a link.
    { label: 'Messenger', Icon: MessengerIcon, className: 'bg-[#0084FF] hover:brightness-110 text-white', copyFallback: true, copyFallbackUrl: 'https://www.messenger.com/' },
    { label: 'Telegram', Icon: Send, className: 'bg-[#229ED9] hover:brightness-110 text-white', href: `https://t.me/share/url?url=${encodedUrl}&text=${encodedIntro}` },
    { label: 'Email', Icon: Mail, className: 'bg-slate-800 hover:bg-slate-700 text-slate-100 border border-slate-700', scheme: 'mailto', href: `mailto:?subject=${encodeURIComponent(`Invitation: ${title}`)}&body=${encodedMessage}` },
  ];
  ...
  const handleShare = async (target: ShareTarget) => {
    ...
    // Native share sheet is the most reliable path — it covers every app the
    // device has (WhatsApp, Messenger, Telegram, Mail, …). Available on phones
    // and on desktop Chrome/Edge over a secure origin (https / localhost).
    if (canNativeShare) {
      try {
        await navigator.share({ title, text: message, url });
        return;
      } catch (err: any) {
        // A user cancel (AbortError) should just close the sheet quietly;
        // any other failure falls through to the per-channel fallback.
        if (err?.name === 'AbortError') return;
      }
    }

    // mailto: must run in the current tab, or the browser opens a dead blank
    // tab and never hands off to the mail client.
    if (target.scheme === 'mailto' && target.href) {
    ...
    // Messenger (or any channel with no usable web link): copy the invite and
    // open the app so the sender can paste it.
    if (target.copyFallback) {
      const ok = await copyText(message);
      if (target.copyFallbackUrl) window.open(target.copyFallbackUrl, '_blank', 'noopener,noreferrer');
      setNotice(
        ok
          ? `Invite copied — paste it into ${target.label}.`
          : `Couldn't copy automatically — use the Copy button above, then paste into ${target.label}.`
      );
    }
  };
    ...
    >
      ...
      >
        ...
        <div>
          ...
          <div className="grid grid-cols-4 gap-2">
            {shareTargets.map((target) => (
              <button
                ...
                className={`flex flex-col items-center justify-center gap-1.5 py-3 rounded-xl font-bold text-[10px] transition-all ${target.className}`}
              >
          ...
          {notice && <p className="text-[11px] text-amber-300 mt-2">{notice}</p>}
        </div>
