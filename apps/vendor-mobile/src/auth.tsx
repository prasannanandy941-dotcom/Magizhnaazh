...
const TOKEN_KEY = 'magizhnaazh_vendor_token';
const USER_KEY = 'magizhnaazh_vendor_user';
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const persist = useCallback(async (u: User, t: string) => {
    // This is the vendor portal — a customer account must not sign in here.
    if (u.role && u.role !== 'vendor' && u.role !== 'admin') {
      throw new Error('This account is not a vendor account. Please use the customer app, or register as a vendor.');
    }
    setUser(u);
    setToken(t);
