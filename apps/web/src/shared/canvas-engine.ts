...
export function renderCanvasToSVG(elements: CanvasElement[], width = 400, height = 600, bgColor = '#1E1B4B'): string {
  ...
  for (const el of sorted) {
    if (el.type === 'text') {
      svgContent += `<text x="${el.x}" y="${el.y + (el.fontSize || 16)}" font-family="${el.fontFamily || 'sans-serif'}" font-size="${el.fontSize || 16}" fill="${el.color || '#ffffff'}" font-weight="600">${el.content || ''}</text>`;
    }
  }
